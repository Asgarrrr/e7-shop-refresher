...
Automation tool for Epic Seven's **Secret Shop** on the STOVE PC
client. Detects bookmark / mystic medal / covenant icons, buys them
automatically, refreshes the shop, repeats.
...
account being banned by Smilegate / STOVE.** The author makes no
warranty and accepts no liability for account losses. Use at your own
risk.
...
Per round (configurable up to `max_refreshes`, or capped by other stop
conditions):
...
4. Click the **Refresh** button, confirm the gem cost.
5. Pause (human-ish delays), repeat.

Stops automatically when any configured stop condition fires
(`max_refreshes`, `stop_after_minutes`, `stop_when_mystic_medals`,
`stop_when_covenants`), when you click **Stop**, or if the shop screen
disappears. If `sleep_when_done` is enabled and a configured goal
fires, the PC is suspended afterwards. Manual Stop never sleeps.

## How it works

- **Capture** — Windows Graphics Capture (via `xcap`) grabs frames of
  the game window without screen-recording the whole desktop.
- **Detection** — Normalised cross-correlation template matching (via
  ...
  they want to buy).
- **Clicks** — `enigo` synthesises mouse motion (curved, eased,
  jittered) and button events at the OS level. The window is brought
  to the foreground first via `SetForegroundWindow` so clicks land
  in-game.
...
- A working display at any resolution you like. The bot calibrates
  against your actual window size; it does not require 1080p.
...
On first launch the GUI tells you what's missing (templates not
cropped, zones not drawn) and refuses to start until everything is
calibrated. Follow the **First-time setup** below.
...
1. **Templates** — small PNGs of each item's icon. You crop them once
   from a live screenshot via the GUI.
2. **Zones** — rectangles you draw around the buttons the bot needs to
   click (refresh, confirm modals, the buy column).
3. **Regions** — search-area rectangles for the templates (shop grid
   for items, top-left header for the anchor). The shipped defaults
   are tuned for 1920×1080 — fine-tune via the GUI if your window is a
   different size and detection misses.

All of this is done in the **Setup** tab.

### Step 1 — Templates

**No templates ship with the project on purpose** — they have to match
your client exactly. Crop three PNGs into the `templates/` folder via
the GUI:

- `shop_header.png` — the Secret Shop banner at the top-left of the
  shop screen.
- `mystic_medal.png` — the small green medal icon (shown in the row
  when a mystic medal is on sale).
- `covenant.png` — the small pink/red bookmark icon.

To create them:

1. Open the secret shop screen in-game.
2. In the GUI's **Snapshot** panel, click **Refresh** — a snapshot of
...
3. **Drag** a rectangle on the snapshot tightly around the icon you
   ...
   background loosens the template match.
...
Repeat for all three. Reference crops live in `templates/examples/`
as visual guides for what a good crop looks like — **do not use them
as your actual templates**; they'll only match the exact resolution
and language they were captured at.

> **Tip:** refresh the shop a few times until an actual mystic medal
> or covenant appears — you need a real icon to crop. The shop header
> is always visible and can be cropped any time.
...
| `refresh_confirm` | The blue Confirm button inside the refresh modal. |
| `buy_confirm` | The large green Buy button inside the buy modal. |
| `buy_column` | The vertical column where every row's small per-row buy button lives. Only the X range matters — the Y is derived from the matched item icon at runtime. |
...
4. The drawn rect is auto-saved to `config.toml`. Coloured overlays
   on the snapshot show what's currently set.
...
where the modal will appear** (centred middle-bottom of the window
for the buy modal; same general area for the refresh modal). The
button will land in that zone when the modal opens at runtime.
...
- `shop_grid` — the rectangle that contains the item rows. Must
  include the icon column, item names, and the buy button column.
...
boxes around what it matched inside these regions. If the boxes fall
on the right items, you're done.

### Step 4 — Run

Once the **Run** tab shows green (no missing templates, no missing
zones), **▶ Start** lights up. Click it. The bot takes over the mouse
and starts buying / refreshing.
...
After first-time setup, daily use is:
...
The `config.toml` is preserved across runs. Slider and toggle changes
in the GUI auto-save with a short debounce.
...
| Section | Key | Effect |
|---|---|---|
| `[shop]` | `max_refreshes` | Stop after N rounds. `0` disables this limit. |
...
| `[shop]` | `buy_button_y_offset_ratio` | Vertical offset (window-height fraction) from the icon centre down to the row's buy button. E7's button sits below the icon. |
| `[shop]` | `max_scrolls_per_round` | How many scroll-downs per round. 6 items fit in 2 views, so 1 is usually enough. |
| `[matching]` | `threshold` | NCC acceptance (0.90 by default). Raise if too many false matches; lower if real items are missed. |
| `[matching]` | `margin` | Required gap between the best and runner-up scores for an unambiguous match. |
| `[timing]` | `click_delay_*` | Inter-click delays — log-normal distribution. |
| `[timing]` | `modal_open_pause_ms` | How long to wait for a modal to render before hashing the confirm zone. |
| `[timing]` | `scroll_amount` / `scroll_pause_ms` | Per-scroll lines and settle time. |
| `[timing]` | `long_pause_every_n` | Take a longer break every N rounds. `0` disables. |
...
| `[window]` | `auto_resize` | Force the window to `base_resolution` at startup. Off by default — fighting Windows decorations + DPI rounding makes 1:1 cropping at native size simpler. |

If at least one of the four stop conditions is set, the bot will stop
when **whichever fires first**. All four at zero = run until manual
Stop.

## Tools

Three binaries ship alongside the GUI:

- **`gui.exe`** — main control panel. What you use day-to-day.
- **`e7-shop-refresher.exe`** — headless CLI runner. Same behaviour
  as the GUI but no window. Useful for scripting. `--dry-run` to
  validate the config without clicking.
...
**"Templates missing"** — the GUI lists which PNGs are expected and
where. Re-crop them via the Crop & Save panel, then hit **Recheck**
on the Templates panel.
...
1. Re-crop the template tighter, focusing on the icon centre only.
...
hash check should skip these silently (you'll see `buy modal did not
open` in the logs). If you see actual phantom purchases, the zones
are bleeding into wrong UI elements; re-draw them tighter.

**Bot freezes for several seconds** — likely an `xcap` WGC frame-pool
...
flag every ~60 ms, so worst case Stop takes ~200 ms. If it takes much
longer, it's stuck in a click animation — the GUI's join is
